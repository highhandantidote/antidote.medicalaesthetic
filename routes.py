...
def index():
    ...
    try:
        ...
        # Removed affordable_packages - section removed from homepage
        ...
        # Get category data for homepage sections
        category_packages = {}
        try:
            categories = ['Injectables & Anti-Aging', 'Facials & Skin Glow', 'Laser & Advanced Technology', 
                         'Hair Treatments', 'Scar, Spots & Skin Correction', 'Surgical Procedures', 
                         'Body Treatments', 'Medical Consultations', 'Brows & Eyes', 'Lips & Smile']
            
            for category in categories:
                packages = db.session.execute(text("""
                    SELECT p.id, p.title, p.slug, p.category, p.description, p.price_actual, p.price_discounted,
                           p.duration, p.downtime, p.actual_treatment_name, c.name as clinic_name, c.slug as clinic_slug
                    FROM packages p
                    JOIN clinics c ON p.clinic_id = c.id
                    JOIN package_categories pc ON p.id = pc.package_id
                    WHERE p.is_active = true AND pc.category_name = :category
                    ORDER BY RANDOM()
                    LIMIT 7
                """), {'category': category}).fetchall()
                
                category_packages[category] = [dict(row._mapping) for row in packages] if packages else []
        except Exception as e:
            logger.error(f"Error loading category packages: {str(e)}")
            category_packages = {}
        ...
        return render_template(
            ...
            category_packages=category_packages  # Category sections data
        )
            ...
            try:
                ...
                for package in packages:
                    ...
                    suggestions.append({
                        ...
                        'category': 'Package',
                        'url': url_for('enhanced_package.package_detail', slug=package.slug),
...
def admin_lead_analytics():
    ...
    try:
        ...
        # Lead query with user information - only match by user_id for safety
        base_query = """
            SELECT 
                l.*,
                COALESCE(c.name, 'Unknown Clinic') as clinic_name,
                COALESCE(u.name, l.patient_name) as patient_name,
                COALESCE(u.phone_number, l.mobile_number, l.phone) as mobile_number,
                COALESCE(u.email, l.email) as email,
                COALESCE(p.procedure_name, l.procedure_name, l.procedure_of_interest) as procedure_name
            FROM leads l
        ...
        for row in raw_leads:
            ...
            # Debug: Log mobile number info for debugging
            logger.debug(f"Lead {lead_dict.get('id')}: mobile_number={lead_dict.get('mobile_number')}")
        ...
        cursor2 = conn.cursor() if not conn.closed else psycopg2.connect(os.environ.get('DATABASE_URL')).cursor()
        ...
        clinic_metrics_query = """
            SELECT 
                ...
                CASE WHEN COUNT(l.id) > 0 
                    THEN ROUND((COUNT(CASE WHEN l.contacted_at IS NOT NULL THEN 1 END)::numeric / COUNT(l.id)) * 100, 2)
                    ELSE 0 END as contact_rate,
                CASE WHEN COUNT(l.id) > 0 
                    THEN ROUND((COUNT(CASE WHEN l.converted_at IS NOT NULL THEN 1 END)::numeric / COUNT(l.id)) * 100, 2)
                    ELSE 0 END as conversion_rate,
                COALESCE(SUM(l.credit_cost), 0) as total_credits_spent,
        ...
        clinic_params = [parsed_start_date, parsed_end_date]
        if clinic_id:
            clinic_metrics_query += " AND c.id = %s"
            clinic_params.append(clinic_id)
        
        clinic_metrics_query += " GROUP BY c.id, c.name, c.credit_balance ORDER BY total_leads DESC"
        
        cursor2.execute(clinic_metrics_query, clinic_params)
        clinic_rows = cursor2.fetchall()
        
        clinic_column_names = [desc[0] for desc in cursor2.description]
        clinic_metrics = []
        for row in clinic_rows:
            clinic_dict = dict(zip(clinic_column_names, row))
            # Calculate ROI percentage
            revenue = clinic_dict.get('total_revenue', 0) or 0
            credits_spent = clinic_dict.get('total_credits_spent', 0) or 0
            if credits_spent > 0:
                clinic_dict['roi_percentage'] = round(((revenue - credits_spent) / credits_spent) * 100, 2)
            else:
                clinic_dict['roi_percentage'] = 0
            clinic_metrics.append(clinic_dict)
...
@web.route('/admin/leads/<int:lead_id>/details')
@login_required
@admin_required
def admin_lead_details(lead_id):
    """Get detailed information for a specific lead."""
    try:
        # Get lead details with user information
        ...
        response_data = {
            'id': lead_data['id'],
            'patient_name': lead_data['patient_name'] or 'Not provided',
            'mobile_number': lead_data['mobile_number'] or 'Not provided',
            'email': lead_data['email'] or 'Not provided',
            'procedure_name': lead_data['procedure_name'] or 'General Inquiry',
            'clinic_name': lead_data['clinic_name'],
            'source_page': lead_data.get('source') or 'Direct',
            'status': lead_data['status'],
            'created_at': lead_data['created_at'].isoformat() if lead_data['created_at'] else None,
            'contacted_at': lead_data.get('contacted_at').isoformat() if lead_data.get('contacted_at') else None,
            'converted_at': lead_data.get('converted_at').isoformat() if lead_data.get('converted_at') else None,
        ...
        logger.error(f"Error getting lead details: {str(e)}")
        return jsonify({'error': f'Error loading lead details: {str(e)}'}), 500
...
def submit_lead():
    ...
    """
    try:
        # Basic spam detection - check for suspicious content
        message = request.form.get('message', '')
        patient_name = request.form.get('patient_name', '')
        
        # Check for spam indicators
        spam_indicators = [
            '<a href=', 'http://', 'https://', 'www.',  # Links
            'лечение', 'запоя', 'интернет', 'провайдер',  # Russian spam words
            'dental', 'tooth powder', 'equipment',  # Common spam topics
            'narkolog', 'tula', 'moscow', 'ekaterinburg'  # Russian cities/medical spam
        ]
        
        content_to_check = (message + ' ' + patient_name).lower()
        for indicator in spam_indicators:
            if indicator.lower() in content_to_check:
                logger.warning(f"Spam detected in lead submission: {indicator} found in content")
        ...
        fake_name_patterns = ['internetkex', 'vivodzapoj', 'stevensix', 'richardhaw', 'daniellal']
        if any(pattern in patient_name.lower() for pattern in fake_name_patterns):
            logger.warning(f"Fake name detected: {patient_name}")
...
def signup():
    """Redirect to new OTP-only authentication system."""
    # Redirect all signup attempts to the new OTP authentication system
    return redirect(url_for('otp_auth.auth_entry'))

@web.route('/login', methods=['GET', 'POST'])
def login():
    """Redirect to new OTP-only authentication system."""
    # Redirect all login attempts to the new OTP authentication system
    return redirect(url_for('otp_auth.auth_entry'))
...
def register_routes(app):
    ...
    # Register OTP-only authentication system
    try:
        from otp_auth import otp_auth, inject_firebase_config
        app.register_blueprint(otp_auth)
        app.context_processor(inject_firebase_config)
        logger.info("OTP-only authentication system registered successfully.")
    except ImportError as e:
        logger.warning(f"OTP authentication system not found: {e}")
