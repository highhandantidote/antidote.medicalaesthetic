#!/usr/bin/env python3
"""
Simplified migration script for Supabase.

This script uses direct connection parameters instead of a connection string
...
SUPABASE_USER = "postgres.asgwdfirkanaaswobuj"
...
def get_supabase_connection():
    """Get a connection to the Supabase database using separate connection parameters."""
    try:
        # Using the provided password
        supabase_password = "Ar8897365503@#"
        
        logger.info("Using provided Supabase password")
        ...
        logger.info(f"Connecting to Supabase ({SUPABASE_HOST})...")
        conn = psycopg2.connect(**conn_params)
        logger.info("Connected to Supabase successfully")
        
        # Build and store the URL format for later reference
        supabase_url = f"postgresql://{SUPABASE_USER}:{supabase_password}@{SUPABASE_HOST}:{SUPABASE_PORT}/{SUPABASE_DBNAME}"
