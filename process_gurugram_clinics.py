#!/usr/bin/env python3
"""
Process Gurugram Google Places clinic data for database import.
This script cleans and transforms the scraped Gurugram clinic data into the format needed for our clinic database.
"""
...
def clean_gurugram_city_name(city: str) -> str:
    """Clean and standardize Gurugram city name."""
    if not city:
        return "Gurugram"
    
    city = city.strip()
    
    # Various forms of Gurugram
    gurugram_variants = ['gurugram', 'gurgaon', 'millennium city']
    
    for variant in gurugram_variants:
        if variant in city.lower():
            return "Gurugram"
    
    return "Gurugram"

def process_gurugram_clinic_data():
    """Process the Gurugram clinic data CSV and create a cleaned version."""
    input_file = "attached_assets/dataset_crawler-google-places_2025-06-20_10-00-12-682_1750413650520.csv"
    output_file = "gurugram_clinics.csv"
    ...
    try:
        with open(input_file, 'r', encoding='utf-8') as infile:
            ...
            for index, row in enumerate(reader, 1):
                ...
                # Clean city name
                city = clean_gurugram_city_name(row.get('city', ''))
                ...
                clinic_data = {
                    ...
                    'city': city,
                    'state': 'Haryana',  # All Gurugram clinics are in Haryana
                    'country': 'India',
                    'pincode': pincode,
                ...
                # Progress indicator
                if index % 50 == 0:
                    print(f"Processed {index} Gurugram clinics...")
        ...
        print(f"\n✅ Successfully processed {len(processed_clinics)} Gurugram clinics")
        print(f"📁 Output file: {output_file}")
        print(f"📊 Data ready for import into clinic database")
        
        # Show sample of processed data
        if processed_clinics:
            print(f"\n📋 Sample Gurugram clinic data:")
