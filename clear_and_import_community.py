#!/usr/bin/env python3
"""
Clear existing community posts and import new ones from CSV.

This script removes all existing community posts and their replies,
then imports the new 34 posts from the CSV file.
"""
...
def clear_existing_community_data(conn):
    """Clear all existing community posts and replies."""
    try:
        with conn.cursor() as cursor:
            ...
            logger.info(f"Clearing {posts_count} existing community posts and {replies_count} replies...")
            
            # Clear replies first (due to foreign key constraints)
            cursor.execute("DELETE FROM community_replies")
            
            # Clear community posts
            cursor.execute("DELETE FROM community")
            
            # Reset sequences to start from 1
            cursor.execute("ALTER SEQUENCE community_id_seq RESTART WITH 1")
            cursor.execute("ALTER SEQUENCE community_replies_id_seq RESTART WITH 1")
            
            conn.commit()
            logger.info(f"Successfully cleared {posts_count} posts and {replies_count} replies")
            
    except Exception as e:
        logger.error(f"Error clearing existing community data: {str(e)}")
        conn.rollback()
        raise
...
def main():
    """Main function to clear existing data and import new community posts."""
    logger.info("Starting community data replacement...")
    ...
    try:
        ...
        clear_existing_community_data(conn)
        ...
        logger.info(f"=== Community data replacement completed successfully ===")
        
        return 0
        
    except Exception as e:
        logger.error(f"Error during community data replacement: {str(e)}")
        return 1
    finally:
        conn.close()
