#!/usr/bin/env python3
"""
Process Chennai Google Places clinic data for database import.
This script cleans and transforms the scraped Chennai clinic data into the format needed for our clinic database.
"""
...
def clean_chennai_city_name(city: str) -> str:
    """Clean and standardize Chennai city name."""
    if not city:
        return "Chennai"
    
    city = city.strip()
    
    # Various forms of Chennai
    chennai_variants = ['chennai', 'madras', 'tamil nadu, chennai']
    
    for variant in chennai_variants:
        if variant in city.lower():
            return "Chennai"
    
    return "Chennai"

def process_chennai_clinic_data():
    """Process the Chennai clinic data CSV and create a cleaned version."""
    input_file = "attached_assets/dataset_crawler-google-places_2025-06-20_10-02-06-487_1750413920910.csv"
    output_file = "chennai_clinics.csv"
    ...
    try:
        with open(input_file, 'r', encoding='utf-8') as infile:
            ...
            for index, row in enumerate(reader, 1):
                ...
                # Clean city name
                city = clean_chennai_city_name(row.get('city', ''))
                ...
                clinic_data = {
                    ...
                    'city': city,
                    'state': 'Tamil Nadu',  # All Chennai clinics are in Tamil Nadu
                    'country': 'India',
                    'pincode': pincode,
                ...
                # Progress indicator
                if index % 50 == 0:
                    print(f"Processed {index} Chennai clinics...")
        ...
        print(f"\n✅ Successfully processed {len(processed_clinics)} Chennai clinics")
        print(f"📁 Output file: {output_file}")
        print(f"📊 Data ready for import into clinic database")
        
        # Show sample of processed data
        if processed_clinics:
            print(f"\n📋 Sample Chennai clinic data:")
