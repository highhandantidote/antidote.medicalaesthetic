#!/usr/bin/env python3
"""
Process Delhi Google Places clinic data for database import.
This script cleans and transforms the scraped Delhi clinic data into the format needed for our clinic database.
"""
...
def clean_delhi_city_name(city: str) -> str:
    """Clean and standardize Delhi city name."""
    if not city:
        return "New Delhi"
    
    city = city.strip()
    
    # Various forms of Delhi
    delhi_variants = ['delhi', 'new delhi', 'old delhi', 'south delhi', 'north delhi', 'east delhi', 'west delhi', 'central delhi']
    
    for variant in delhi_variants:
        if variant in city.lower():
            return "New Delhi"
    
    return "New Delhi"

def process_delhi_clinic_data():
    """Process the Delhi clinic data CSV and create a cleaned version."""
    input_file = "attached_assets/dataset_crawler-google-places_2025-06-20_09-50-15-786_1750413147782.csv"
    output_file = "delhi_clinics.csv"
    ...
    try:
        with open(input_file, 'r', encoding='utf-8') as infile:
            ...
            for index, row in enumerate(reader, 1):
                ...
                # Clean city name
                city = clean_delhi_city_name(row.get('city', ''))
                ...
                clinic_data = {
                    ...
                    'city': city,
                    'state': 'Delhi',  # All Delhi clinics are in Delhi state/UT
                    'country': 'India',
                    'pincode': pincode,
                ...
                # Progress indicator
                if index % 100 == 0:
                    print(f"Processed {index} Delhi clinics...")
        ...
        print(f"\n✅ Successfully processed {len(processed_clinics)} Delhi clinics")
        print(f"📁 Output file: {output_file}")
        print(f"📊 Data ready for import into clinic database")
        
        # Show sample of processed data
        if processed_clinics:
            print(f"\n📋 Sample Delhi clinic data:")
