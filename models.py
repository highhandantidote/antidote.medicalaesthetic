...
class User(UserMixin, db.Model):
    ...
    name = Column(Text, nullable=True)  # Made nullable to support OTP-only auth flow
    ...
    # Profile completion fields for 2-step auth (TEMPORARILY COMMENTED OUT - DB COLUMNS DON'T EXIST)
    # age = Column(Integer)  # User age
    # gender = Column(Text)  # User gender
    # city = Column(Text)  # User city/location
    # interests = Column(ARRAY(Text))  # Areas of interest
    # profile_completed = Column(Boolean, default=False)  # Profile completion status
    # NOTE: Profile data is stored in bio field as JSON until DB migration is possible
...
class Package(db.Model):
    ...
    description = Column(Text)
    actual_treatment_name = Column(String(200))  # Technical treatment name for display in brackets
    ...
    procedure_breakdown = Column(JSON)  # Detailed procedure breakdown with pricing
    ...
    package_categories = relationship("PackageCategory", back_populates="package", cascade="all, delete-orphan")
    
    # Helper property to get category names as a list
    @property
    def categories(self):
        """Get list of category names for this package."""
        return [pc.category_name for pc in self.package_categories]
    
    # Helper property for backward compatibility
    @property
    def primary_category(self):
        """Get the first category for backward compatibility."""
        return self.categories[0] if self.categories else self.category
    
    def __repr__(self):
        return f"<Package {self.title} by {self.clinic.name if self.clinic else 'Unknown'}>"

class PackageCategory(db.Model):
    """Junction table for many-to-many relationship between packages and categories."""
    ...
    # Relationships
    package = relationship("Package", back_populates="package_categories")
    
    def __repr__(self):
        return f"<PackageCategory package_id={self.package_id} category={self.category_name}>"
