"""
Process a single thread to add replies.
This script handles just one thread to ensure it completes without timeout.
"""
import os
import logging
import psycopg2
...
def get_one_thread_without_replies():
    """Get exactly one thread with no replies."""
    conn = None
    try:
        ...
        with conn.cursor() as cursor:
            ...
            if result:
                ...
                logger.info(f"Found thread to process: {thread_id} - '{title}'")
                thread_info = (thread_id, title)
            else:
...
def add_replies_to_thread(thread_id, thread_title):
    ...
    try:
        ...
        with conn.cursor() as cursor:
            cursor.execute("SELECT MAX(id) FROM community_replies")
            result = cursor.fetchone()
            last_reply_id = result[0] if result and result[0] is not None else 0
...
def process_single_thread():
    """Process just one thread to add replies."""
    # Get one thread without replies
    thread_info = get_one_thread_without_replies()
    if not thread_info:
        return 0
    
    thread_id, title = thread_info
    num_added = add_replies_to_thread(thread_id, title)
    
    if num_added > 0:
        logger.info(f"Successfully processed thread {thread_id}")
    else:
        logger.error(f"Failed to add replies to thread {thread_id}")
    ...
    return 1 if num_added > 0 else 0
