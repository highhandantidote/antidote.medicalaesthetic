#!/usr/bin/env python3
"""
Process Kolkata Google Places clinic data for database import.
This script cleans and transforms the scraped Kolkata clinic data into the format needed for our clinic database.
"""
...
def clean_kolkata_city_name(city: str) -> str:
    """Clean and standardize Kolkata city name."""
    if not city:
        return "Kolkata"
    
    city = city.strip()
    
    # Various forms of Kolkata
    kolkata_variants = ['kolkata', 'calcutta']
    
    for variant in kolkata_variants:
        if variant in city.lower():
            return "Kolkata"
    
    return "Kolkata"

def process_kolkata_clinic_data():
    """Process the Kolkata clinic data CSV and create a cleaned version."""
    input_file = "attached_assets/dataset_crawler-google-places_2025-06-20_10-00-06-970_1750413774035.csv"
    output_file = "kolkata_clinics.csv"
    ...
    try:
        with open(input_file, 'r', encoding='utf-8') as infile:
            ...
            for index, row in enumerate(reader, 1):
                ...
                # Clean city name
                city = clean_kolkata_city_name(row.get('city', ''))
                ...
                clinic_data = {
                    ...
                    'city': city,
                    'state': 'West Bengal',  # All Kolkata clinics are in West Bengal
                    'country': 'India',
                    'pincode': pincode,
                ...
                # Progress indicator
                if index % 50 == 0:
                    print(f"Processed {index} Kolkata clinics...")
        ...
        print(f"\n✅ Successfully processed {len(processed_clinics)} Kolkata clinics")
        print(f"📁 Output file: {output_file}")
        print(f"📊 Data ready for import into clinic database")
        
        # Show sample of processed data
        if processed_clinics:
            print(f"\n📋 Sample Kolkata clinic data:")
