#!/usr/bin/env python3
"""
Generate community thread replies for the Antidote platform.

This script:
1. Creates doctor user accounts if they don't exist
2. Adds 2-4 replies to each thread created by generate_community_threads.py
3. Preserves all existing data
4. Creates CSV files for import validation

Usage:
    python generate_community_replies.py
"""
import os
import sys
import csv
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("generate_community_replies.log"),
...
def get_threads_to_reply(conn):
    """Get threads to add replies to."""
    threads = []
    
    try:
        # Try to load thread IDs from the JSON file first
        data_dir = Path("community_data")
        json_path = data_dir / "thread_ids.json"
        
        if json_path.exists():
            with open(json_path, 'r', encoding='utf-8') as f:
                thread_ids = json.load(f)
                
            logger.info(f"Loaded {len(thread_ids)} thread IDs from {json_path}")
            
            # Get thread details from database
            with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cursor:
                placeholders = ','.join(['%s'] * len(thread_ids))
                cursor.execute(f"""
                    SELECT c.id, c.title, c.content, c.created_at, c.procedure_id, 
            ...
            # If the file doesn't exist, get the most recent 125 threads
            with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cursor:
                cursor.execute("""
                    SELECT c.id, c.title, c.content, c.created_at, c.procedure_id, 
                           p.procedure_name, u.username, u.email
        ...
        logger.info(f"Found {len(threads)} threads to add replies to")
        return threads
    except Exception as e:
...
def create_thread_replies(conn, threads, patient_user_ids, doctor_user_ids):
    ...
    try:
        ...
        for batch_start in range(0, len(threads), batch_size):
            logger.info(f"Processing batch {batch_start//batch_size + 1}/{(len(threads)+batch_size-1)//batch_size}")
            ...
            with conn.cursor() as cursor:
                ...
                for thread_id, count in thread_reply_counts.items():
                    cursor.execute(
                        "UPDATE community SET reply_count = reply_count + %s WHERE id = %s",
                        (count, thread_id)
                    )
            
            conn.commit()
            replies.extend(batch_replies)
            logger.info(f"Created {len(batch_replies)} replies in batch {batch_start//batch_size + 1}")
        
        logger.info(f"Successfully created {len(replies)} replies for {len(threads)} threads")
        return replies
    except Exception as e:
        conn.rollback()
...
def main():
    """Main function to generate community replies."""
    logger.info("=== Starting community replies generation ===")
    ...
    try:
        ...
        # Get threads to reply to
        threads = get_threads_to_reply(conn)
        if not threads:
            logger.error("Failed to get threads to reply to")
            return 1
        ...
        logger.info("=== Community replies generation completed successfully ===")
        return 0
    except Exception as e:
