...
This script handles special characters in the connection URL and migrates
all data from your current database to Supabase.
"""
import os
import sys
import json
...
def get_supabase_connection():
    """Get a connection to the Supabase database using a properly parsed URL."""
    try:
        ...
        logger.info("Parsing Supabase connection URL...")
        
        try:
            # Try a direct connection first
            conn = psycopg2.connect(supabase_url)
            logger.info("Connected to Supabase successfully using direct URL")
            return conn
        except Exception as direct_error:
            logger.warning(f"Direct connection failed: {str(direct_error)}")
            logger.info("Trying to parse connection URL manually...")
            
            # Manual parsing as a fallback
            if "postgresql://" not in supabase_url:
                logger.error("URL does not start with postgresql://")
                return None
                
            # Extract components
            parts = supabase_url.replace("postgresql://", "").split("@")
            if len(parts) != 2:
                logger.error("URL does not contain @ separator")
                return None
                
            credentials = parts[0].split(":")
            if len(credentials) != 2:
                logger.error("Credentials part does not contain username:password format")
                return None
                
            username = credentials[0]
            password = ":".join(credentials[1:])  # Handle passwords with colons
            
            host_parts = parts[1].split("/")
            if len(host_parts) < 2:
                logger.error("Host part does not contain /dbname format")
                return None
            ...
            dbname = "/".join(host_parts[1:])  # Handle database names with slashes
            ...
            logger.info(f"Connecting to Supabase host: {host}...")
            conn = psycopg2.connect(**conn_params)
            logger.info("Connected to Supabase successfully using parsed parameters")
            return conn
...
def migrate_database():
    ...
    try:
        ...
        if 'SUPABASE_DATABASE_URL' in os.environ:
            update_env_file(os.environ['SUPABASE_DATABASE_URL'])
