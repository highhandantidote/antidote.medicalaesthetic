#!/usr/bin/env python3
"""
Add procedures directly to the database in micro-batches of 5 procedures.

Each procedure is added in its own transaction to avoid timeout issues.
"""
...
START_ROW = 25  # Skip rows that are already imported (we have 24 existing procedures)
MAX_ROWS = 5    # Tiny batches to ensure success
...
def add_single_procedure(conn, procedure):
    """Add a single procedure with its own transaction."""
    if not conn:
        return False
    ...
    try:
        ...
        with conn.cursor() as cur:
            ...
            # Commit immediately for this procedure
            conn.commit()
            return True
...
def import_procedures():
    """Import procedures from CSV using tiny batches with immediate commits."""
    ...
    try:
        ...
        # Import each procedure individually with its own transaction
        procedures_added = 0
        for procedure in procedures:
            if add_single_procedure(conn, procedure):
                procedures_added += 1
