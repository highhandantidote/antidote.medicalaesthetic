#!/usr/bin/env python3
"""
Process Mumbai Google Places clinic data for database import.
This script cleans and transforms the scraped Mumbai clinic data into the format needed for our clinic database.
"""
...
def clean_mumbai_city_name(city: str) -> str:
    """Clean and standardize Mumbai city name."""
    if not city:
        return "Mumbai"
    
    city = city.strip()
    
    # Various forms of Mumbai
    mumbai_variants = ['mumbai', 'bombay', 'greater mumbai', 'mumbai metropolitan region']
...
def process_mumbai_clinic_data():
    """Process the Mumbai clinic data CSV and create a cleaned version."""
    input_file = "attached_assets/dataset_crawler-google-places_2025-06-20_09-40-36-558_1750412562606.csv"
    output_file = "mumbai_clinics.csv"
    ...
    try:
        with open(input_file, 'r', encoding='utf-8') as infile:
            ...
            for index, row in enumerate(reader, 1):
                ...
                # Clean city name
                city = clean_mumbai_city_name(row.get('city', ''))
                ...
                clinic_data = {
                    ...
                    'city': city,
                    'state': 'Maharashtra',  # All Mumbai clinics are in Maharashtra
                    'country': 'India',
                    'pincode': pincode,
                ...
                # Progress indicator
                if index % 100 == 0:
                    print(f"Processed {index} Mumbai clinics...")
        ...
        print(f"\n✅ Successfully processed {len(processed_clinics)} Mumbai clinics")
        print(f"📁 Output file: {output_file}")
        print(f"📊 Data ready for import into clinic database")
        
        # Show sample of processed data
        if processed_clinics:
            print(f"\n📋 Sample Mumbai clinic data:")
