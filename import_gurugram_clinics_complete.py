#!/usr/bin/env python3
"""
Complete Gurugram clinic import from CSV with Google Places API integration.
Imports all 92 Gurugram clinics with authentic Google Business data.
"""
...
def create_unique_identifiers(name: str, imported_count: int) -> tuple:
    ...
    email = f'{clean_name}ggn{imported_count}{timestamp}@clinic.antidote.com'
    
    base_slug = re.sub(r'[^a-z0-9-]', '', name.lower().replace(' ', '-'))[:12]
    if not base_slug:
        base_slug = 'gurugram-clinic'
...
def import_gurugram_clinics_complete():
    """Import all Gurugram clinics from CSV with Google Places API integration."""
    ...
    # Load Gurugram CSV data
    gurugram_records = []
    csv_file_path = 'attached_assets/gurugram_filtered_clinics.csv - Sheet1_1750458443860.csv'
    ...
    with open(csv_file_path, 'r', encoding='utf-8') as file:
        ...
        for row in reader:
            ...
            if place_id and name and place_id not in existing_places:
                gurugram_records.append(row)
    
    total_records = len(gurugram_records)
    print(f"Starting Gurugram import: {total_records} clinics to process")
    
    imported_count = 0
    failed_count = 0
    
    # Process each clinic
    for i, record in enumerate(gurugram_records, 1):
        ...
        try:
            ...
            clinic_data = {
                'name': name,
                'slug': slug,
                'description': f'{name} - Professional medical aesthetics clinic in Gurugram',
                'address': address,
                'city': 'Gurugram',
                'state': 'Haryana',
                'country': 'India',
    ...
    print(f"Gurugram: {gurugram_total} clinics")
...
if __name__ == "__main__":
    import_gurugram_clinics_complete()