#!/usr/bin/env python3
"""
Process Ahmedabad Google Places clinic data for database import.
This script cleans and transforms the scraped Ahmedabad clinic data into the format needed for our clinic database.
"""
...
def clean_ahmedabad_city_name(city: str) -> str:
    """Clean and standardize Ahmedabad city name."""
    if not city:
        return "Ahmedabad"
    
    city = city.strip()
    
    # Various forms of Ahmedabad
    ahmedabad_variants = ['ahmedabad', 'amdavad', 'ahmadabad']
    
    for variant in ahmedabad_variants:
        if variant in city.lower():
            return "Ahmedabad"
    
    return "Ahmedabad"

def process_ahmedabad_clinic_data():
    """Process the Ahmedabad clinic data CSV and create a cleaned version."""
    input_file = "attached_assets/dataset_crawler-google-places_2025-06-20_10-06-13-513_1750414072003.csv"
    output_file = "ahmedabad_clinics.csv"
    ...
    try:
        with open(input_file, 'r', encoding='utf-8') as infile:
            ...
            for index, row in enumerate(reader, 1):
                ...
                # Clean city name
                city = clean_ahmedabad_city_name(row.get('city', ''))
                ...
                clinic_data = {
                    ...
                    'city': city,
                    'state': 'Gujarat',  # All Ahmedabad clinics are in Gujarat
                    'country': 'India',
                    'pincode': pincode,
                ...
                # Progress indicator
                if index % 50 == 0:
                    print(f"Processed {index} Ahmedabad clinics...")
        ...
        print(f"\n✅ Successfully processed {len(processed_clinics)} Ahmedabad clinics")
        print(f"📁 Output file: {output_file}")
        print(f"📊 Data ready for import into clinic database")
        
        # Show sample of processed data
        if processed_clinics:
            print(f"\n📋 Sample Ahmedabad clinic data:")
