...
def index():
    ...
    try:
        try:
            ...
            if popular_body_parts is None:
                popular_body_parts = []
            # Ensure we always have a list, never None
            if not isinstance(popular_body_parts, list):
                popular_body_parts = list(popular_body_parts) if popular_body_parts else []
        except Exception as e:
            logger.error(f"Error querying body parts: {str(e)}")
            popular_body_parts = []
        
        try:
            # List of specific high-demand procedure names
            high_demand_procedures = [
                'Botox', 'Lip Fillers', 'Liposuction', 
                'Gynecomastia', 'Rhinoplasty', 'Chemical Peel'
            ]
            
            # Query specifically those procedures by name
            popular_procedures = Procedure.query.filter(
                Procedure.procedure_name.in_(high_demand_procedures)
            ).all()
            
            # Ensure we have a list
            if popular_procedures is None:
                popular_procedures = []
            # Ensure we always have a list, never None
            if not isinstance(popular_procedures, list):
                popular_procedures = list(popular_procedures) if popular_procedures else []
                
            # Sort them in the order specified in the list
            if popular_procedures:
                # Create a mapping of procedure name to index in high_demand_procedures list
                procedure_order = {name: idx for idx, name in enumerate(high_demand_procedures)}
                # Sort procedures based on their position in the high_demand_procedures list
                popular_procedures.sort(key=lambda p: procedure_order.get(p.procedure_name, 999))
            
            # If we don't have all the procedures, log this information
            if len(popular_procedures) < len(high_demand_procedures):
                logger.info(f"Only found {len(popular_procedures)} out of {len(high_demand_procedures)} high-demand procedures")
                
            # If we don't have any high-demand procedures at all, fall back to popular ones
            if not popular_procedures:
                logger.info("No high-demand procedures found, falling back to most popular")
                fallback_procedures = Procedure.query.order_by(Procedure.popularity_score.desc()).limit(6).all()
                popular_procedures = fallback_procedures if fallback_procedures is not None else []
        except Exception as e:
        ...
        try:
            recent_threads = Community.query.order_by(Community.created_at.desc()).limit(3).all()
            if recent_threads is None:
                recent_threads = []
            # Ensure we always have a list, never None
            if not isinstance(recent_threads, list):
                recent_threads = list(recent_threads) if recent_threads else []
        except Exception as e:
        ...
        try:
            ...
            if top_doctors is None:
                top_doctors = []
            # Ensure we always have a list, never None
            if not isinstance(top_doctors, list):
                top_doctors = list(top_doctors) if top_doctors else []
        except Exception as e:
            logger.error(f"Error querying doctors: {str(e)}")
            top_doctors = []
            
        # Get ALL procedures for booking forms (since all doctors can handle all procedures)
        try:
            ...
            if not isinstance(all_procedures, list):
                all_procedures = list(all_procedures) if all_procedures else []
        except Exception as e:
            logger.error(f"Error querying all procedures: {str(e)}")
            all_procedures = []
        
        # If we don't have enough doctors with ratings, get some without filtering by rating
        # Simplified query - no fallback needed
        
        # Final safety check - ensure all variables are safe for len()
        safe_popular_procedures = popular_procedures if popular_procedures is not None else []
        safe_recent_threads = recent_threads if recent_threads is not None else []
        safe_top_doctors = top_doctors if top_doctors is not None else []
        safe_procedure_categories = procedure_categories if procedure_categories is not None else []
        safe_popular_body_parts = popular_body_parts if popular_body_parts is not None else []
        safe_all_procedures = all_procedures if all_procedures is not None else []
        
        # Log what we're displaying
        logger.info(f"Index page showing: {len(safe_popular_procedures)} procedures, {len(safe_recent_threads)} threads, {len(safe_top_doctors)} doctors, {len(safe_procedure_categories)} categories")
        ...
        if show_modern_design:
            ...
            return render_template(
                'ultra-index.html',
                procedures=safe_popular_procedures,
                doctors=safe_top_doctors,
                threads=safe_recent_threads,
                body_parts=safe_popular_body_parts,
                procedure_categories=safe_procedure_categories,
                all_procedures=safe_all_procedures
            )
        else:
            # Use the original template
            return render_template(
                'index.html',
                popular_body_parts=safe_popular_body_parts,
                popular_procedures=safe_popular_procedures,
                top_doctors=safe_top_doctors,
                recent_threads=safe_recent_threads,
                procedure_categories=safe_procedure_categories,
                all_procedures=safe_all_procedures
            )
    except Exception as e:
        logger.error(f"Error rendering index page: {str(e)}")
        # Return safe empty lists for all variables
...
def autocomplete():
    ...
    Returns JSON data with procedure, doctor, or thread suggestions that match the query.
    
    Query parameters:
    - q: Search query (required)
    - type: Search type ('doctors', 'procedures', 'discussions', or blank for all)
    """
    try:
...
def search():
    ...
    try:
        ...
        if search_type not in ['doctors', 'procedures', 'threads']:
            search_type = None
...
@web.route('/community')
def community():
    """Render the new Reddit-style modern community interface."""
...
def doctors():
    ...
    try:
        ...
        if sort_by == 'experience_desc':
            doctors = base_query.order_by(Doctor.experience.desc().nulls_last()).all()
            title += " - Sorted by Experience (Most First)"
        elif sort_by == 'experience_asc':
            doctors = base_query.order_by(Doctor.experience.asc().nulls_last()).all()
            title += " - Sorted by Experience (Least First)"
        elif sort_by == 'rating_desc':
            doctors = base_query.order_by(Doctor.rating.desc().nulls_last()).all()
            title += " - Sorted by Rating (Highest First)"
        elif sort_by == 'fee_asc':
            doctors = base_query.order_by(Doctor.consultation_fee.asc().nulls_last()).all()
            title += " - Sorted by Fee (Lowest First)"
        elif sort_by == 'fee_desc':
            doctors = base_query.order_by(Doctor.consultation_fee.desc().nulls_last()).all()
            title += " - Sorted by Fee (Highest First)"
        elif sort_by == 'name_asc':
            doctors = base_query.order_by(Doctor.name.asc()).all()
...
def register_routes(app):
    ...
    # Import Simple AI recommendation routes
    try:
        from simple_ai_recommendation import simple_ai
    ...
    # Import Gemini AI recommendation routes
    try:
        from gemini_ai_recommendation import gemini_ai
        logger.info("Gemini AI recommendation routes imported successfully.")
    except ImportError:
        logger.warning("Gemini AI recommendation routes not found. Gemini AI recommendation features will not be available.")
        gemini_ai = None
    ...
    if simple_ai:
        app.register_blueprint(simple_ai)
        logger.info("AI recommendations blueprint registered successfully.")
        
    # Register Gemini AI recommendation routes
    if gemini_ai:
        app.register_blueprint(gemini_ai)
        logger.info("Gemini AI recommendations blueprint registered successfully.")
    ...
    # Register enhanced community blueprint
    if community_bp:
