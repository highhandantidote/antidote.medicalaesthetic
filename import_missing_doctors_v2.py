#!/usr/bin/env python3
"""
Import missing doctors from CSV to database with correct database schema.

This script adds doctors that are in the CSV but not in the database,
using the actual database structure from the project.
"""
    ...
    try:
        ...
        for data in csv_doctors:
            ...
            try:
                ...
                practice_location = data.get('Address', '').strip()
                city = data.get('city', '').strip()
                state = data.get('state', '').strip()
                
                # Insert doctor - using ONLY columns that actually exist in the table
                with conn.cursor() as cursor:
                    cursor.execute(
                        """
                        INSERT INTO doctors (
                            name, user_id, specialty, education,
                            experience, bio, practice_location, city, state,
