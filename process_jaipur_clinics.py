#!/usr/bin/env python3
"""
Process Jaipur Google Places clinic data for database import.
This script cleans and transforms the scraped Jaipur clinic data into the format needed for our clinic database.
"""
...
def clean_jaipur_city_name(city: str) -> str:
    """Clean and standardize Jaipur city name."""
    if not city:
        return "Jaipur"
    
    city = city.strip()
    
    # Various forms of Jaipur
    jaipur_variants = ['jaipur', 'pink city']
    
    for variant in jaipur_variants:
        if variant in city.lower():
            return "Jaipur"
    
    return "Jaipur"

def process_jaipur_clinic_data():
    """Process the Jaipur clinic data CSV and create a cleaned version."""
    input_file = "attached_assets/dataset_crawler-google-places_2025-06-20_10-15-19-430_1750414605924.csv"
    output_file = "jaipur_clinics.csv"
    ...
    try:
        with open(input_file, 'r', encoding='utf-8') as infile:
            ...
            for index, row in enumerate(reader, 1):
                ...
                # Clean city name
                city = clean_jaipur_city_name(row.get('city', ''))
                ...
                clinic_data = {
                    ...
                    'city': city,
                    'state': 'Rajasthan',  # All Jaipur clinics are in Rajasthan
                    'country': 'India',
                    'pincode': pincode,
                ...
                # Progress indicator
                if index % 25 == 0:
                    print(f"Processed {index} Jaipur clinics...")
        ...
        print(f"\n✅ Successfully processed {len(processed_clinics)} Jaipur clinics")
        print(f"📁 Output file: {output_file}")
        print(f"📊 Data ready for import into clinic database")
        
        # Show sample of processed data
        if processed_clinics:
            print(f"\n📋 Sample Jaipur clinic data:")
