#!/usr/bin/env python3
"""
Complete Kolkata Clinics Import System
Import all 438 Kolkata clinics from CSV with Google Places API verification
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(f'kolkata_import_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log'),
...
def create_unique_identifiers(name, imported_count):
    ...
    email = f'{clean_name}kolkata{imported_count}{timestamp}@clinic.antidote.com'
    
    # Create slug
    base_slug = re.sub(r'[^a-z0-9-]', '', name.lower().replace(' ', '-'))[:15]
    if not base_slug:
        base_slug = 'kolkata-clinic'
...
def import_single_clinic(record, api_key, imported_count):
    ...
    try:
        ...
        specialties = record.get('specialties', 'Medical Aesthetics, Cosmetic Surgery, Dental Care')
        ...
        cursor.execute('''
            ...
            RETURNING id
        ''', (
            clinic_name, slug, f'{clinic_name} - Professional medical aesthetics clinic in Kolkata',
            address, 'Kolkata', 'West Bengal', 'India', record.get('pincode'),
...
def import_kolkata_clinics():
    """Import all Kolkata clinics from CSV file."""
    api_key = get_google_api_key()
    if not api_key:
        logging.error("Google Places API key not found")
        return
    
    # Load Kolkata clinic data
    csv_file = 'attached_assets/kolkata_filtered_clinics.csv - Sheet1_1750460363298.csv'
    
    try:
        with open(csv_file, 'r', encoding='utf-8') as file:
            reader = csv.DictReader(file)
            kolkata_records = list(reader)
    except Exception as e:
        logging.error(f"Error reading CSV file: {e}")
        return
    
    total_records = len(kolkata_records)
    logging.info(f"Starting Kolkata import: {total_records} clinics to process")
    
    imported_count = 0
    success_count = 0
    
    for record in kolkata_records:
        if import_single_clinic(record, api_key, imported_count):
            success_count += 1
        
        imported_count += 1
        
        # Progress update every 30 clinics
        if imported_count % 30 == 0:
            logging.info(f"Progress: {imported_count}/{total_records} processed | {success_count} successful")
    
    # Final statistics
    success_rate = (success_count / total_records) * 100 if total_records > 0 else 0
    
    logging.info(f"\n=== KOLKATA IMPORT COMPLETE ===")
    ...
    # City-wise counts
    cursor.execute('SELECT COUNT(*) FROM clinics WHERE city = %s', ('Kolkata',))
    kolkata_total = cursor.fetchone()[0]
    ...
    logging.info(f"Kolkata: {kolkata_total} clinics")
    ...
    logging.info(f"TOTAL: {total_clinics} clinics across 7 major Indian cities")
